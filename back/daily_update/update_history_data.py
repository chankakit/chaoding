...
def get_stock_a_hist_and_write(stock_code, start_date_str, end_date_str, bk_name, db_conn):
  ...
  stock_zh_a_hist_df.to_sql(table_name, db_conn, if_exists="replace")
...
def get_bk_part_data(stock_bk_part, start_date_str, end_date_str, bk_name, db_path):
  ...
  with sql.connect(database_file) as db_conn:
    for index, stock_code in enumerate(stock_bk_part['company_code']):
      ...
      try:
        get_stock_a_hist_and_write(stock_code, start_date_str, end_date_str, bk_name, db_conn)
        if index % 30 == 29:
...
def get_a_stock_data(start_date_str, end_date_str, mp=False):
  ...
  with sql.connect(__a_stk_ls_db) as a_stk_db_conn:
    for bk in bks:
      ...
      if mp:
        ...
        pl = get_bk_stock_data_mp(bk_data, start_date_str, end_date_str, bk, __a_stk_hist_blocks_dir)
        pls.append(pl)
      else:
    ...
    print('update history data done.')
...
if __name__=='__main__':
  ...
  get_a_stock_data(start_date, end_date, True)
